...
class AssignTicketUseCaseTest {
    ...
    private AssignTicketUseCase useCase;
    ...
    void setUp() {
        ...
        useCase = new AssignTicketUseCase(ticketRepository, projectRepository, eventBus);
        ...
        project = Project.builder()
                ...
                .members(List.of(
                        ...
                        new Member(3, assigneeId, "assignee", "", Permission.VIEWER)
                ))
    ...
    @Test
    void testExecute_AssignsPlayerSuccessfully() {
        ...
        assertTrue(result.getAssignees().contains(assigneeId));
        verify(ticketRepository).save(any(Ticket.class));
        verify(eventBus).publish(any(TicketAssignedEvent.class));
    }

    @Test
    void testExecute_ThrowsIfNotProjectMember() {
        when(ticketRepository.findById(10L)).thenReturn(Optional.of(ticket));
        when(projectRepository.findById(1L)).thenReturn(Optional.of(project));

        UUID nonMemberId = UUID.randomUUID();
        assertThrows(IllegalArgumentException.class, () -> useCase.execute(10L, editorId, nonMemberId));
        verify(ticketRepository, never()).save(any());
        verify(eventBus, never()).publish(any());
    }

    @Test
    void testExecute_ThrowsIfAlreadyAssigned() {
        Ticket alreadyAssigned = new Ticket(10L, 1L, 1, "Test", "", "Task", "OPEN",
                Priority.MEDIUM, List.of(assigneeId), Collections.emptyList(),
                editorId, Collections.emptyList(), 0L, 0L, null);
        when(ticketRepository.findById(10L)).thenReturn(Optional.of(alreadyAssigned));
        when(projectRepository.findById(1L)).thenReturn(Optional.of(project));
    ...
    @Test
    void testExecute_PublishesTicketAssignedEvent() {
        when(ticketRepository.findById(10L)).thenReturn(Optional.of(ticket));
        when(projectRepository.findById(1L)).thenReturn(Optional.of(project));
        when(ticketRepository.save(any(Ticket.class))).thenAnswer(i -> i.getArguments()[0]);
