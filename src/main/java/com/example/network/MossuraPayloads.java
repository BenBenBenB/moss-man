...
public final class MossuraPayloads {
	...
	public record CreateTicketPayload(UUID projectId, String title, String description, String type, String state, TicketPriority priority, String assigneeName) implements CustomPayload {
		...
		private static CreateTicketPayload read(RegistryByteBuf buf) {
			...
			return new CreateTicketPayload(projectId, title, description, type, state, priority, assigneeName);
		}
	...
	public record UpdateTicketPayload(UUID projectId, UUID ticketId, String title, String description, String type, String state, TicketPriority priority, String assigneeName) implements CustomPayload {
		...
		private static UpdateTicketPayload read(RegistryByteBuf buf) {
			...
			boolean hasAssignee = buf.readBoolean();
			String assigneeName = hasAssignee ? buf.readString(64) : null;
			return new UpdateTicketPayload(projectId, ticketId, title, description, type, state, priority, assigneeName);
		}
	...
	public record UpdateProjectSettingsPayload(UUID projectId, String name, List<String> statuses, List<String> ticketTypes) implements CustomPayload {
		...
		private static UpdateProjectSettingsPayload read(RegistryByteBuf buf) {
			...
			return new UpdateProjectSettingsPayload(projectId, name, statuses, types);
		}
	...
	public record AddProjectMemberPayload(UUID projectId, String memberName) implements CustomPayload {
		...
		private static AddProjectMemberPayload read(RegistryByteBuf buf) {
			return new AddProjectMemberPayload(buf.readUuid(), buf.readString(64));
		}
