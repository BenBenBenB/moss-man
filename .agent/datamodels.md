...
| `relatorDescription` | `String` | Description of the relation (e.g., "needs"). |
| `relateeDescription` | `String` | Description for the inverse (e.g., "is needed by"). |
...
| :--- | :--- | :--- |
| `id` | `UUID` | Unique internal identifier. |
| `projectId` | `UUID` | Reference to the parent project ID. |
...
| `relationships` | `List<Relationship>` | Links to other tickets. |
| `createdAt` | `long` | Unix timestamp (ms). |
| `updatedAt` | `long` | Unix timestamp (ms). |
| `sprintId` | `UUID?` | Optional reference to a sprint. |
...
| `targetTicketId` | `UUID` | The ID of the ticket on the other side. |
